...
> **프로젝트 상태:** 기능 프리뷰 버전입니다. 6개 도구가 Tauri 클라이언트에서 로컬로 작동하며 핵심 변환과 간결한 데스크톱 작업 흐름에 집중합니다.
...
- 라이트 / 다크 테마
- 영어 / 중국어 UI 전환
- 환경설정 저장, 도구 검색, 즐겨찾기, 복사, 지우기, 다운로드
...
| 도구 | 상태 | 설명 |
| --- | --- | --- |
| Base64 | 사용 가능 | UTF-8 / URL-safe 인코딩 및 디코딩, 기록, 복사, 지우기, 다운로드 |
| MD5 | 사용 가능 | 텍스트와 파일 해시, 대소문자 전환, 검증 |
| 타임스탬프 | 사용 가능 | 초 / 밀리초, 시간대, 실시간 시계, 복사 |
| JSON 포맷터 | 사용 가능 | 포맷팅, 압축, 들여쓰기, 키 정렬, 복사, 다운로드 |
| URL 인코더 / 디코더 | 사용 가능 | 구성 요소 / 전체 URL 모드, 복사, 지우기, 교환 |
| UUID 생성기 | 사용 가능 | RFC 4122 v4 UUID 하나 이상 생성 |
...
모든 변환은 클라이언트에서 로컬로 처리되며 입력 데이터를 업로드하지 않습니다. MD5는 단방향 해시이며 복호화할 수 없습니다.
