...
function detectPlatform() {
  if (typeof window !== 'undefined' && window.__TAURI_OS_PLUGIN_INTERNALS__) {
    try {
      ...
      if (current === 'macos' || current === 'windows' || current === 'linux') return current;
    } catch (error) {
      console.warn('PouchTools platform detection failed', error);
    }
  }
  if (typeof navigator !== 'undefined') {
    const signature = `${navigator.userAgent || ''} ${navigator.platform || ''}`.toLowerCase();
    if (/macintosh|mac os x|darwin/.test(signature)) return 'macos';
    if (/windows/.test(signature)) return 'windows';
  }
  return 'linux';
}
...
function WindowControls({ platformName, maximized, onMinimize, onToggleMaximize, onClose, t }) {
  const controls = <>
    <IconButton label={t('最小化', 'Minimize')} className="window-control window-minimize" onClick={onMinimize}><Minus size={17} /></IconButton>
    <IconButton label={maximized ? t('还原窗口', 'Restore window') : t('最大化', 'Maximize')} className="window-control window-maximize" onClick={onToggleMaximize}>{maximized ? <Minimize2 size={17} /> : <Maximize2 size={17} />}</IconButton>
    <IconButton label={t('关闭窗口', 'Close window')} className="window-control window-close" onClick={onClose}><X size={17} /></IconButton>
  </>;
  if (platformName === 'macos') {
    return <div className="window-controls macos-controls" onMouseDown={(event) => event.stopPropagation()}>
      <IconButton label={t('关闭窗口', 'Close window')} className="macos-control macos-close" onClick={onClose}><X size={10} /></IconButton>
      <IconButton label={t('最小化', 'Minimize')} className="macos-control macos-minimize" onClick={onMinimize}><Minus size={10} /></IconButton>
      <IconButton label={maximized ? t('还原窗口', 'Restore window') : t('最大化', 'Maximize')} className="macos-control macos-maximize" onClick={onToggleMaximize}>{maximized ? <Minimize2 size={10} /> : <Maximize2 size={10} />}</IconButton>
    </div>;
  }
  return <div className={`window-controls ${platformName}-controls`} onMouseDown={(event) => event.stopPropagation()}>{controls}</div>;
}
...
function App() {
  ...
  const [platformName] = useState(detectPlatform);
